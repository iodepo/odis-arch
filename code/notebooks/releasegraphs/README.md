...
The implementation process might involve GitHub LFS and actions, or it could be generated and published via the OIH object store. We are currently investigating the best approach. We're also considering modifications to the generation and storage of RDF and the associated PROV RDF.

Initial exploratory code, which tests the use of NLP for extracting GPEs from these resources, can be found in [./dataScience](dataScience/README.md). This marks the beginning of our exploration into potential ways to leverage the OIH Graph.
## Assets
