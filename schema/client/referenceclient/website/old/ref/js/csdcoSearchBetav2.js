...
const showresults = (content) => {
	...
	for (var i = 0; i < count; i++) {
		...
		if (getSafe(() => barval[i].name.value)) {
			headTemplate.push(html` <span style="float:left;margin-left:10px"><a href="${barval[i].s.value}">${barval[i].name.value}</a> 
			 a ${barval[i].type.value}</span>  <span style="float:right;margin-right:5px"> (score: ${barval[i].score.value}) </span> `);
		}
