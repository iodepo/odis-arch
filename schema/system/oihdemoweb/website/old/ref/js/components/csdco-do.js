    ...
    class FCTest extends HTMLElement {
        constructor() {
            ...
            const compacted = jsonld.compact(obj, context).then((providers) => {
                ...
                else detailsTemplate.push(html`<div><a  href='${jp["@graph"][1]["url"]}' download='${jp["@graph"][1]["https://schema.org/name"]}'>Browser Download Link</a> </div>
                          <div> URL: <a href='${jp["@graph"][1]["url"]}'>${jp["@graph"][1]["url"]}</a> </div>`);
